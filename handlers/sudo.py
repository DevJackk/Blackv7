...
def sudo(client, message,redis):
	...
	if text and (type is "private" or (type is "supergroup" or type is "group")) :
		...
		if rank is "sudo" or rank is "asudo":
			...
			if text == c.ADDfiles:
				url = "https://raw.githubusercontent.com/jack1eljoker1/Blackv7-files/master/files"
			...
			if text == c.Ubot:
				...
				for fnh in Files_H:
					url = "https://raw.githubusercontent.com/jack1eljoker1/Blackv7/master/handlers/"+fnh
					out = requests.get(url).text
					f = open("./handlers/"+fnh,"w+")
					f.write(out)
					f.close()
				for fnu in Files_U:
					url = "https://raw.githubusercontent.com/jack1eljoker1/Blackv7/master/utlis/"+fnu
					out = requests.get(url).text
					f = open("./utlis/"+fnu,"w+")
					f.write(out)
					f.close()
				for fnb in Files_B:
					url = "https://raw.githubusercontent.com/jack1eljoker1/Blackv7/master/"+fnb
					out = requests.get(url).text
					f = open("./"+fnb,"w+")
					f.write(out)
					f.close()
				for fnu in Files_L:
					url = "https://raw.githubusercontent.com/jack1eljoker1/Blackv7/master/lang/"+fnu
					out = requests.get(url).text
					f = open("./lang/"+fnu,"w+")
					f.write(out)
					f.close()
